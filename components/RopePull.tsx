...
export default function RopePull() {
  ...
  return (
    ...
    >
      ...
      {/* Rope */}
      <svg width="20" height={ROPE_HEIGHT} viewBox={`0 0 20 ${ROPE_HEIGHT}`} fill="none" overflow="visible">
        <line x1="10" y1="0" x2="10" y2={ROPE_HEIGHT}
      ...
      <motion.div style={{ y: ringY }} className="flex flex-col items-center">
        ...
        <motion.div whileHover={{ scale: 1.22 }} transition={{ type: "spring", stiffness: 400, damping: 18 }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            {isDark ? (
              <path d="M14 8 A6 6 0 1 0 14 16 A4 4 0 1 1 14 8" fill={accent} opacity="0.85" />
            ) : (
              <>
      ...
      {/* "pull me bro" tooltip — appears on hover */}
      <motion.div
        ...
        className="pointer-events-none absolute bottom-12 right-8 whitespace-nowrap rounded-lg dark:border dark:border-d-border border border-l-border dark:bg-d-card bg-l-card px-3 py-1.5 shadow-lg"
      >
        <span className="font-mono text-[10px] dark:text-d-accent text-l-accent">
          pull me bro
        </span>
        {/* little arrow pointing right toward rope */}
        <span className="absolute right-[-5px] top-1/2 -translate-y-1/2 dark:border-l-d-border border-l-l-border"
          style={{
            width: 0, height: 0,
